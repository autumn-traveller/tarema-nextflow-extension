...
class ReportObserver implements TraceObserver {
    ...
    private void sendDataToDB(TaskRun task, TraceRecord trace) {
        ...
        def params = [taskName, trace.get("%cpu"), trace.get("rss"), trace.get("peak_rss"), trace.get("vmem"), trace.get("peak_vmem"), trace.get("rchar"), trace.get("wchar"), task.getConfig().getCpus(), task.getConfig().getMemory().getBytes(), trace.get("realtime"), false, session.runName, task.container]
