...
class State {
    ...
    boolean equals(Object obj) {
        return (obj != null) && (obj instanceof State) && (obj.c == this.c)
    }
}
...
class QAgent {
    ...
    public QAgent(int initialCpu, int maxCpu,String taskName, String command, String runName, boolean withLogs){
        ...
        this.state = new State(this.cpuOptions[currentCpuInd])
    ...
    private void readPrevRewards() {
        ...
        def searchSql = "SELECT taskrun.id,cpus,cpu_usage,realtime,taskrun.duration FROM taskrun LEFT JOIN runs ON taskrun.run_name = runs.run_name WHERE task_name = (?) and taskrun.rl_active = true and taskrun.id > (?) and (command = (?) or taskrun.run_name = (?)) ORDER BY created_at asc"
        sql.eachRow(searchSql,[taskName, lastTaskId, command, runName]) { row ->
            ...
            switch(actionTaken){
                case NOOP:
                    prevState = new State(cpus)
                    break;
                case INCR_CPU:
                    prevState = new State(this.cpuOptions[cpuInd-1])
                    break;
                case DECR_CPU:
                    prevState = new State(this.cpuOptions[cpuInd+1])
                    break;
