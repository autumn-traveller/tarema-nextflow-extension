...
//                        stepSize = 0.001
    ...
    private void updateCpuPreferences(int cpus, int realtime){
        def r = reward(realtime)
        logInfo("Task \"$taskName\": cpus alloc'd $cpus, realtime $realtime -> reward $r (avg reward so far: $cpuAvgReward)\n")
        for (i in 0..<maxCpu) {
            if (i == cpus - 1){
    ...
    private void readPrevRewards() {
        ...
        def sql = new Sql(TaskDB.getDataSource())
        def searchSql = "SELECT id,cpus,realtime FROM taskrun WHERE task_name = (?) and rl_active = true and id > (?) order by created_at asc"
        sql.eachRow(searchSql,[taskName,lastTaskId]) { row ->
            ...
            updateCpuPreferences(cpus,realtime)
